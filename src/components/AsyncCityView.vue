            ...
            <div class="mx-8 text-white">
                ...
                <div v-for="day in weatherData.daily" :key="day.dt" class="flex items-center">
                    ...
                    <div class="flex gap-2 flex-1 justify-end">
                        <p>Highest: {{ Math.round(day.temp.max) }}</p>
                        <p>Lowest: {{ Math.round(day.temp.min) }}</p>
                    </div>
                </div>
            </div>
        </div>
...
const getWeatherData = async () => {
    try {
        const weatherData = await axios.get(
            `https://api.openweathermap.org/data/2.5/onecall?lat=${route.query.lat}&lon=${route.query.lng}&units=metric&exclude={part}&appid=7efa332cf48aeb9d2d391a51027f1a71`
        );
